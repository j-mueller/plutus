...
echo "Checking out $BRANCH branch into $WORKTREE_NAME"
git worktree add -B $BRANCH $WORKTREE_NAME $UPSTREAM_BRANCH
...
echo "If you are happy with the output in $WORKTREE_NAME, then change into that directory and push it to $UPSTREAM_BRANCH"
