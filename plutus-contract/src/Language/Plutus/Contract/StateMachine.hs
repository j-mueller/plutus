...
import           Ledger.Constraints               (ScriptLookups, TxConstraints (..), mustPayToScript)
...
runInitialise StateMachineClient{scInstance} initialState initialValue = do
    let StateMachineInstance{validatorInstance} = scInstance
        tx = mustPayToScript initialState initialValue
    let lookups = Constraints.scriptLookups validatorInstance
